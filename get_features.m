...
sf = waveletScattering('SignalLength',3000,'SamplingFrequency',300);
...
try
    ...
    for i = [leads_idx]
       ...
       % features(jj) = RSM(i);
        ...
        for i=[leads_idx]
            res_ecg(i,:)=resample(filt_ecg(i,:),300,Fs);
        end
        Fs=300;
    ...
    try
        ...
        qrs=qrs_detect2(normalize(res_data(2,:)),0.25, 0.6, Fs);
    end 
   ...
   scattTrain=[];
    ...
    scattTrain=zeros(features_length,152);
    fbfeat=zeros(1,20);
end
